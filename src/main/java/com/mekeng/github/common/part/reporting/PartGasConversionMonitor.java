...
public class PartGasConversionMonitor extends AbstractPartGasMonitor {
    ...
    private void drainGasContainer(final EntityPlayer player, final EnumHand hand) {
        try {
            ...
            final IMEMonitor<IAEGasStack> cell = this.getProxy().getStorage().getInventory(AEApi.instance().storage().getStorageChannel(IGasStorageChannel.class));
            ...
            if (notInserted != null && notInserted.getStackSize() > 0) {
                MekEng.log.error("Gas item [%s] reported a different possible amount to drain than it actually provided.", held.getDisplayName());
            }
